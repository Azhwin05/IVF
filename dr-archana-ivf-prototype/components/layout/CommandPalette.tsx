        ...
        <div className="scroll-area max-h-[380px] p-2">
          {results.length === 0 && (
            <div className="px-3 py-10 text-center">
              <p className="text-[13.5px] text-ink-500">No results for “{q}”</p>
              <p className="mt-1 text-[12px] text-ink-400">Try a patient name, screen or embryo ID.</p>
            </div>
          )}
          {results.map((r, i) => {
            ...
            return (
              <div key={r.id}>
                {showGroup && (
                  <p className="px-2.5 pb-1 pt-3 text-[10.5px] font-semibold uppercase tracking-[0.12em] text-ink-400 first:pt-1">
                    {r.group}
                  </p>
                )}
                <button
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={cn('truncate text-[13.5px] font-medium', active ? 'text-brand-900' : 'text-ink-800')}>
                      {r.label}
                    </p>
                    <p className="truncate text-[11.5px] text-ink-400">{r.hint}</p>
                  </div>
